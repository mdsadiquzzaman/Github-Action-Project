...
Use the POST /api/auth/login route with the seeded user credentials:
Email: admin@example.com
Password: 123456
Copy the token from the response.
Click the 🔒 Authorize button at the top right of the Swagger UI.
Enter the token in the format: Bearer <your_token_here> and click Authorize.
You can now test the Items CRUD endpoints.
API Endpoints
Auth
